        ...
        <div className="h-52 flex items-end justify-between gap-2 px-2">
          {analytics.monthlyUsers.map((v, i) => (
            <div key={i} className="flex-1 flex flex-col items-center gap-1">
              <motion.div initial={{ height: 0 }} animate={{ height: `${(v / Math.max(...analytics.monthlyUsers, 1)) * 100}%` }} transition={{ delay: i * 0.04, duration: 0.5 }}
                className="w-full bg-gradient-to-t from-emerald-500 to-emerald-400 rounded-t-lg relative group cursor-pointer min-h-[2px]">
        ...
        <motion.div variants={item} className="glass-card p-6">
          ...
          <div className="h-44 flex items-end justify-between gap-2 px-2">
            {analytics.monthlyUploads.map((v, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <motion.div initial={{ height: 0 }} animate={{ height: `${(v / Math.max(...analytics.monthlyUploads, 1)) * 100}%` }} transition={{ delay: i * 0.04, duration: 0.5 }}
                  className="w-full bg-gradient-to-t from-brand-500 to-brand-400 rounded-t-lg relative group cursor-pointer min-h-[2px]">
        ...
        <motion.div variants={item} className="glass-card p-6">
          ...
          <div className="space-y-4">
            {analytics.topSubjects.map((s, i) => {
              ...
              const maxCount = Math.max(...analytics.topSubjects.map(x => x.count), 1);
              return (
                <div key={s.subject}>
                  ...
                  <div className="h-3 bg-gray-100 dark:bg-dark-surface rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${(s.count / maxCount) * 100}%` }} transition={{ delay: 0.3 + i * 0.1 }}
      ...
      <motion.div variants={item} className="glass-card p-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          {leaderboard.length === 0 && (
            <p className="text-sm text-gray-400 text-center py-4 col-span-full">No data yet</p>
          )}
        </div>
      </motion.div>
    </motion.div>
