...
const tierConfig = {
  ...
  default: { color: 'from-gray-300 to-gray-500', text: 'text-gray-400', bg: 'bg-gray-100 dark:bg-gray-800', border: 'border-gray-400/30', icon: Medal },
};

const TIER_THRESHOLDS = { Elite: 1000, Gold: 500, Silver: 200, Bronze: 0 };
...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      try {
        const resp = await api.get('/api/v1/leaderboard?page_size=50');
        setLeaderboard(resp.rankings || []);
      } catch (err) {
  ...
  if (loading) {
    return <div className="flex justify-center py-12"><div className="w-8 h-8 rounded-full border-2 border-brand-500 border-t-transparent animate-spin"/></div>;
  }
    ...
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6">
      ...
      {currentUserRank > 0 && (
        <motion.div variants={item} className="glass-card p-5 flex items-center gap-4 border-l-4 border-brand-500">
          ...
          <div className="ml-auto text-right">
            ...
            <span className={`badge ${tierConfig[user?.tier || 'Bronze']?.bg || tierConfig.default.bg} ${tierConfig[user?.tier || 'Bronze']?.text || tierConfig.default.text} text-xs font-bold`}>
              {user?.tier || 'Bronze'}
            </span>
          </div>
        </motion.div>
      ...
      <motion.div variants={item} className="flex items-end justify-center gap-4 py-8">
        ...
        {top3[1] && (
          <motion.div variants={item} className="text-center">
            <div className="w-16 h-16 mr-2 ml-2 rounded-full bg-gradient-to-br from-gray-300 to-gray-500 flex items-center justify-center mx-auto mb-2 border-2 border-gray-300 shadow-lg">
        ...
        {top3[2] && (
          <motion.div variants={item} className="text-center">
            <div className="w-16 h-16 ml-2 mr-2 rounded-full bg-gradient-to-br from-orange-400 to-orange-600 flex items-center justify-center mx-auto mb-2 border-2 border-orange-300 shadow-lg">
      ...
      <motion.div variants={item} className="glass-card overflow-hidden">
        ...
        {leaderboard.length === 0 && (
          <div className="p-8 text-center text-gray-500">No ranked users yet!</div>
        )}
        
        {leaderboard.map((entry) => {
          const cfg = tierConfig[entry.tier || 'Bronze'] || tierConfig.default;
          const isCurrentUser = entry.user_id === user?.id;
          return (
            <div key={entry.user_id} className={`table-row px-6 py-3 flex items-center ${isCurrentUser ? 'bg-brand-50/50 dark:bg-brand-900/10' : ''}`}>
              ...
              <div className="flex-1 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400">Rating: {entry.average_rating ? entry.average_rating.toFixed(1) : 0}</p>
                </div>
              </div>
              <span className="w-24 text-center text-sm text-gray-600 dark:text-gray-400 hidden sm:block">{entry.uploads_count || 0}</span>
              <span className="w-24 text-center text-sm text-gray-600 dark:text-gray-400 hidden sm:block">{entry.downloads_count || 0}</span>
